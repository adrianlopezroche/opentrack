...
/* Detect ArUco MIP 36h12 and AprilTag MIP 36h11 markers. This likely belongs in
 * opentrack/aruco, but for now do it here.
 */
...
namespace arucohead {
    ...
    */
    ...
    /* Match marker bits against dictionary, returning the marker ID and hamming distance.
    */
    int getDictionaryMatch(uint64_t extractedWord, const std::vector<uint64_t>& dict, int &distance) {
        distance = std::numeric_limits<int>::max();
        int id = -1;

        for (size_t i = 0; i < dict.size(); i++) {
            int dist = hammingDistance(extractedWord, dict[i]);
    ...
    */
    int analyzeMarkerImage6x6(const cv::Mat &grey, int &nRotations, const std::vector<uint64_t> &dict, int maxHammingDistance = 2)
    {
        // Inner grid is 6x6. Total grid size (including the black border) is 8x8.
        const int innerSize = 6;
        const int gridSize = innerSize + 2;
        int cellWidth = grey.rows / gridSize;

        // Make sure the border is black.
        for (int y = 0; y < gridSize; y++) {
            ...
            for (int x = 0; x < gridSize; x += inc) {
                cv::Mat square = grey(cv::Rect(x * cellWidth, y * cellWidth, cellWidth, cellWidth));

                if (countNonZero(square) > (cellWidth * cellWidth) / 2)
                    return -1; // Border not black
            }
        }
        ...
        for (int y = 0; y < innerSize; y++) {
            for (int x = 0; x < innerSize; x++) {
                cv::Mat square = grey(cv::Rect((x + 1) * cellWidth, (y + 1) * cellWidth, cellWidth, cellWidth));
        ...
        // Look up each of the four 90-degree rotations in the dictionary, keeping the best match.
        int bestId = -1;
        int bestRotation = -1;
        int bestDistance = std::numeric_limits<int>::max();

        for (int r = 0; r < 4; r++) {
            // Convert the current matrix rotation to a 64-bit integer.
            uint64_t currentWord = 0;
            for (int y = 0; y < innerSize; y++) {
                for (int x = 0; x < innerSize; x++) {
                    currentWord <<= 1;

                    if (bits.at<uchar>(y, x))
                        currentWord |= 1;
                }
            }

            // Find the closest match for the current rotation.
            int distance = 0;
            int id = getDictionaryMatch(currentWord, dict, distance);

            if (id != -1 && distance < bestDistance) {
                bestId = id;
                bestRotation = r;
                bestDistance = distance;

                if (distance == 0)
                    break; // found an exact match, so exit early.
            }
        ...
        // Validate marker.
        if (bestId != -1 && bestDistance <= maxHammingDistance) {
            nRotations = bestRotation;
            return bestId;
        }
    ...
    /* Run threshold on marker image.
    */
    cv::Mat marker_threshold(const cv::Mat &in) {
    ...
    /* Detect ArUco MIP 36h12 marker.
    */
    int detect_aruco_mip_36h12(const cv::Mat &in, int &nRotations) {
        static const std::vector<uint64_t> dict = {
